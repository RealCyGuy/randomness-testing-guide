...
export function autocorrelationTest(bitsPerSymbol: number) {
  return function _(randomString: string) {
    // Extract overlapping chunks (slide by 1 instead of jumping by bitsPerSymbol)
    const words: string[] = [];
    for (let i = 0; i <= randomString.length - bitsPerSymbol; i++) {
